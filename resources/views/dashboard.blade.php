                            ...
                            @foreach($chats as $chat)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('admin.chats.show', $chat->id) }}" class="text-blue-600 hover:text-blue-900 mr-2">
                                            Просмотреть
                                        </a>
                                        <a href="{{ route('admin.chats.edit', $chat->id) }}" class="text-green-600 hover:text-green-900 mr-2">
                                            Редактировать
                                        </a>
                                        <form action="{{ route('admin.chats.destroy', $chat->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
